...
All current project links in `src/components/Projects.tsx` are the literal placeholder `#`. This documentation mirrors that code; ticket `PORT-011` requests real URLs.
...
Provide: status, problem, audience, Julian's contribution, dates, outcome/evidence, stack actually used, repository/live/case-study URLs, media with attribution, privacy/publication status, and last verified date.
