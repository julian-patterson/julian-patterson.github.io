...
- Use a restrained, reduced-motion-aware text scramble on every mounted section kicker (currently six). Ignore the initial observer sample so off-screen labels animate on actual viewport entry; keep the accessible label and reduced-motion rendering static.
...
- Keep the GitHub contribution section dormant and unmounted, as requested in PORT-054. Preserve its component, static data route, workflow wiring, and styles for possible future reuse, but do not render an Activity section unless Julian explicitly restores it.
